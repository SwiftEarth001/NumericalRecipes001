...
int main(int argc, char *argv[])
{
    ...
    for (int m=0; m<4; m++) {
		...
		int norm=2;  // l2 norm;
        double tol = 10e-7;
        descentMethods->GaussSouthwellsolve(*b, *x, &iter, &err, 2, tol, itmax);

        // write results to file;
        std::ofstream test03file;
        test03file.open( output_file[m] );
        for (int k=0; k<N; k++) {
            test03file << std::to_string((*b)[k]) << " ";
        }
        test03file << "\n";
        for (int k=0; k<N; k++) {
            test03file << std::to_string((*x)[k]) << " ";
        }
        test03file << "\n";
        test03file << "iterations: " << std::to_string(iter) << "\n";
        test03file << "error: " << std::to_string(err) << "\n";
        test03file.close();
