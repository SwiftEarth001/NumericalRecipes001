...
int main(int argc, char *argv[])
{
    ...
    for (int m=0; m<4; m++) {
        ...
        std::ofstream test02file;
        test02file.open( output_file[m] );
        for (int k=0; k<N; k++) {
            test02file << std::to_string((*b)[k]) << " ";
        }
        test02file << "\n";
        for (int k=0; k<N; k++) {
            test02file << std::to_string((*x)[k]) << " ";
        }
        test02file << "\n";
        test02file << "iterations: " << std::to_string(iter) << "\n";
        test02file << "error: " << std::to_string(err) << "\n";
        test02file.close();
