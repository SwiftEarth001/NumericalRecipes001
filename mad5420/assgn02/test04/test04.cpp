...
int main(int argc, char *argv[])
{
    ...
    for (int m=0; m<4; m++) {
		...
		int norm=2;  // l2 norm;
        double tol = 10e-7;
        Jacobi(*A, *b, *x, &iter, &err, 2, tol, itmax);

        // write results to file;
        std::ofstream test04file;
        test04file.open( output_file[m] );
        for (int k=0; k<N; k++) {
            test04file << std::to_string((*b)[k]) << " ";
        }
        test04file << "\n";
        for (int k=0; k<N; k++) {
            test04file << std::to_string((*x)[k]) << " ";
        }
        test04file << "\n";
        test04file << "iterations: " << std::to_string(iter) << "\n";
        test04file << "error: " << std::to_string(err) << "\n";
        test04file.close();
