...
int main(int argc, char *argv[])
{
    ...
    IncLeastSqrs* stream_data_model = NULL;
    double** init_model = NULL;
    double* xvar = NULL;
        ...
        if (argv[1][0] == '-') {
            ...
            if (argv[1][1] == 'i') {
                ...
                while (std::getline(*strm_ptr, line)) {
                    ...
                    if (id == numcols-1) {
                        init_model = new double*[numcols-1];
                        for (int i=0; i<numcols-1; i++) { 
                            init_model[i] = new double[numcols]; 
                        }
                        for (int i=0; i<numcols-1; i++) {
                            for (int j=0; j<numcols; j++) {
                                init_model[i][j] = all_doubles[i][j];
                            }
                        }
                        stream_data_model = new IncLeastSqrs(numrows, numcols, init_model, numcols-1);
                        ...
                        if ((id-numcols+1) % (update_count) == 0) {
                            ...
                            for (int k=0; k < (id<numcols-1 ? id : numcols-1); k++) {
                                outfile << xvar[k] << " ";
                            }
                            outfile << std::endl;
                        }
            ...
            if (argv[1][1] == 'f') {
                ...
                while (std::getline(inFile, line)) {
                    ...
                    ++id;
                    if (id == numcols-1) {
                        init_model = new double*[numrows];
                        for (int k=0; k<numcols; k++) { 
                            init_model[k] = new double[numcols]; 
                        }
                        ...
                        stream_data_model = new IncLeastSqrs(numrows, numcols, init_model, numrows);
                        while (all_doubles.size() > 0) { 
                            all_doubles.pop_back(); 
                        }
                    } else if (id >= numcols) {
    ...
    if (stream_data_model != NULL) { delete stream_data_model; }
